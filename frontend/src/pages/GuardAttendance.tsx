...
export default function GuardAttendance() {
  ...
  const [studentsByDate, setStudentsByDate] = useState<{
    [date: string]: Student[];
  }>({});
  ...
  useEffect(() => {
    const fetchAttendanceData = async () => {
      try {
        ...
        // Batch students by date
        const batchedStudents = mergedData.reduce(
          (acc: { [date: string]: Student[] }, student) => {
        ...
        setStudentsByDate(batchedStudents);
      } catch (error) {
              ...
              <div className="space-y-4">
                <div className="rounded-md border">
                  <Table>
                    ...
                    <TableBody>
                      {Object.keys(studentsByDate).map((date) => (
                        <TableRow key={date}>
                          <TableCell colSpan={6}>
                            <Table>
                              <TableBody>
                                {studentsByDate[date].map((student) => (
